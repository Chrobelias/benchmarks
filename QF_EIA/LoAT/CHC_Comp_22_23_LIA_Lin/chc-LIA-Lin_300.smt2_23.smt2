(set-info :status unsat) ; swine result
(set-logic QF_EIA)
(declare-fun i1 () Int)
(declare-fun it103 () Int)
(declare-fun it104 () Int)
(declare-fun it74 () Int)
(declare-fun it105 () Int)
(declare-fun it106 () Int)
(declare-fun it107 () Int)
(declare-fun it108 () Int)
(declare-fun it109 () Int)
(declare-fun it148 () Int)
(declare-fun it149 () Int)
(declare-fun it150 () Int)
(declare-fun it151 () Int)
(declare-fun it152 () Int)
(declare-fun it153 () Int)
(declare-fun it154 () Int)
(declare-fun it155 () Int)
(declare-fun it158 () Int)
(declare-fun it159 () Int)
(declare-fun it160 () Int)
(declare-fun it161 () Int)
(declare-fun it162 () Int)
(declare-fun it163 () Int)
(declare-fun it164 () Int)
(declare-fun it172 () Int)
(declare-fun it173 () Int)
(declare-fun it174 () Int)
(declare-fun it175 () Int)
(declare-fun it176 () Int)
(declare-fun it177 () Int)
(declare-fun it178 () Int)
(declare-fun it179 () Int)
(declare-fun it189 () Int)
(declare-fun it190 () Int)
(declare-fun it191 () Int)
(declare-fun it192 () Int)
(declare-fun it193 () Int)
(declare-fun it194 () Int)
(declare-fun it195 () Int)
(assert (= (+ i1 (- 1)) 0))
(assert (= (+ it103 (- 3)) 0))
(assert (= (+ it104 (- 1)) 0))
(assert (= (+ it105 (* it74 (- 1))) 0))
(assert (= (+ it106 (- 1)) 0))
(assert (= (+ it107 (- 1)) 0))
(assert (= (+ (* it74 (- 1)) it108) 0))
(assert (= (+ it109 (- 1)) 0))
(assert (and (>= (+ it105 (* it148 (- 1)) (* it104 (- 1)) 1) 0)
     (>= (+ (* it148 (- 1)) (* it107 (- 1)) it108 1) 0)
     (>= (+ it103 (- 3)) 0)
     (>= (+ (* it103 (- 1)) 3) 0)
     (>= (+ it148 (- 1)) 0)))
(assert (= (+ it149 (* it103 (- 1))) 0))
(assert (= (+ it150 (* it148 (- 1)) (* it104 (- 1))) 0))
(assert (= (+ (* it105 (- 1)) it151) 0))
(assert (= (+ (* (+ 0 it106) (** 5 it148) (+ 0 (- 1))) (+ 0 it152))
   (+ 0 0)))
(assert (= (+ it153 (* it148 (- 1)) (* it107 (- 1))) 0))
(assert (= (+ it154 (* it108 (- 1))) 0))
(assert (= (+ (* (+ 0 it109) (** 5 it148) (+ 0 (- 1))) (+ 0 it155))
   (+ 0 0)))
(assert (and (= (+ it149 (- 3)) 0)
     (< (+ (* it153 (- 1)) it154) 0)
     (< (+ (* it150 (- 1)) it151) 0)))
(assert (= (+ it158 (- 2)) 0))
(assert (= it159 0))
(assert (= (+ it160 (* it151 (- 1))) 0))
(assert (= (+ it161 (* it152 (- 1))) 0))
(assert (= (+ it162 (- 1)) 0))
(assert (= (+ it163 (* it154 (- 1))) 0))
(assert (= (+ it164 (* it155 (- 1))) 0))
(assert (and (>= (+ it158 (- 2)) 0)
     (>= (+ (* it159 (- 1)) it160 (* it172 (- 1)) 1) 0)
     (>= (+ (* it158 (- 1)) 2) 0)
     (>= (+ (* it162 (- 1)) it163 (* it172 (- 1)) 1) 0)
     (>= (+ it172 (- 1)) 0)))
(assert (= (+ it173 (* it158 (- 1))) 0))
(assert (= (+ it174 (* it159 (- 1)) (* it172 (- 1))) 0))
(assert (= (+ it175 (* it160 (- 1))) 0))
(assert (= (+ (+ 0 (* it159 it172 (- 2)))
      (* (** it172 2) (+ 0 (- 1)))
      (+ 0 (* it176 2))
      (+ 0 it172)
      (+ 0 (* it161 (- 2))))
   (+ 0 0)))
(assert (= (+ (* it162 (- 1)) it177 (* it172 (- 1))) 0))
(assert (= (+ (* it163 (- 1)) it178) 0))
(assert (= (+ (* (** it172 2) (+ 0 (- 1)))
      (+ 0 (* it162 it172 (- 2)))
      (+ 0 (* it164 (- 2)))
      (+ 0 (* it179 2))
      (+ 0 it172))
   (+ 0 0)))
(assert (and (< (+ (* it177 (- 1)) it178) 0)
     (>= (+ (* it174 (- 1)) it175) 0)
     (= (+ it173 (- 2)) 0)))
(assert (= (+ it189 (- 2)) 0))
(assert (= (+ (* it174 (- 1)) it190 (- 1)) 0))
(assert (= (+ (* it175 (- 1)) it191) 0))
(assert (= (+ (* it174 (- 1)) (* it176 (- 1)) it192) 0))
(assert (= (+ it193 (* it177 (- 1))) 0))
(assert (= (+ it194 (* it178 (- 1))) 0))
(assert (= (+ (* it179 (- 1)) it195) 0))
(assert (let ((a!1 (or (> (+ it192 (* it195 (- 1))) 0) (< (+ it192 (* it195 (- 1))) 0))))
  (and a!1
       (= (+ it189 (- 2)) 0)
       (< (+ (* it193 (- 1)) it194) 0)
       (< (+ (* it190 (- 1)) it191) 0))))
(check-sat)
