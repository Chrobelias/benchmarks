(set-info :status sat) ; swine result
(set-logic QF_EIA)
(declare-fun i1 () Int)
(declare-fun it85 () Int)
(declare-fun i3 () Int)
(declare-fun it86 () Int)
(declare-fun it87 () Int)
(declare-fun i4 () Int)
(declare-fun it88 () Int)
(declare-fun i5 () Int)
(declare-fun it89 () Int)
(declare-fun i6 () Int)
(declare-fun i7 () Int)
(declare-fun it90 () Int)
(declare-fun it91 () Int)
(declare-fun i8 () Int)
(declare-fun it219 () Int)
(declare-fun it220 () Int)
(declare-fun it221 () Int)
(declare-fun it222 () Int)
(declare-fun it223 () Int)
(declare-fun it224 () Int)
(declare-fun it225 () Int)
(declare-fun it115 () Int)
(declare-fun it246 () Int)
(declare-fun it247 () Int)
(declare-fun it248 () Int)
(declare-fun it249 () Int)
(declare-fun it250 () Int)
(declare-fun it251 () Int)
(declare-fun it252 () Int)
(declare-fun it255 () Int)
(declare-fun it256 () Int)
(declare-fun it257 () Int)
(declare-fun it258 () Int)
(declare-fun it8 () Int)
(declare-fun it7 () Int)
(declare-fun it259 () Int)
(declare-fun it260 () Int)
(declare-fun it261 () Int)
(assert (= (+ i1 (- 7)) 0))
(assert (= (+ it85 (- 6)) 0))
(assert (= (+ it86 (* i3 (- 1))) 0))
(assert (= (+ (* i4 (- 1)) it87) 0))
(assert (= (+ (* i5 (- 1)) it88) 0))
(assert (= (+ (* i6 (- 1)) it89) 0))
(assert (= (+ it90 (* i7 (- 1))) 0))
(assert (= (+ (* i8 (- 1)) it91) 0))
(assert (= (+ it85 (- 6)) 0))
(assert (= (+ it219 (- 4)) 0))
(assert (= (+ (* it86 (- 1)) it220) 0))
(assert (= (+ it221 (* it87 (- 1))) 0))
(assert (= (+ it222 (* it88 (- 1))) 0))
(assert (= (+ it223 (* it89 (- 1))) 0))
(assert (= (+ it224 (* it90 (- 1))) 0))
(assert (= (+ it225 (* it91 (- 1))) 0))
(assert (let ((a!1 (+ (+ 0 (+ it115 it225))
              (* (+ 0 it224) (** 2 (+ it115 (- 1))) (+ 0 (- 1)))
              (+ 0 (- 2)))))
  (and (>= a!1 (+ 0 0))
       (>= (+ it224 (- 3)) 0)
       (>= (+ it219 (- 4)) 0)
       (>= (+ (* it219 (- 1)) 4) 0)
       (>= (+ it115 (- 1)) 0)
       (>= (+ (* it224 (- 1)) it225 (- 1)) 0))))
(assert (= (+ it246 (* it219 (- 1))) 0))
(assert (let ((a!1 (+ (* (+ 0 it224) (** 2 (+ it115 (- 1))) (+ 0 (- 1)))
              (+ 0 it247))))
  (= a!1 (+ 0 0))))
(assert (= (+ (* it115 (- 1)) (* it225 (- 1)) it248 1) 0))
(assert (= (+ it249 (* it222 (- 1))) 0))
(assert (= (+ it250 (* it223 (- 1))) 0))
(assert (= (+ (* (** 2 it115) (+ 0 it224) (+ 0 (- 1))) (+ 0 it251))
   (+ 0 0)))
(assert (= (+ (* it115 (- 1)) it252 (* it225 (- 1))) 0))
(assert (and (<= (+ it252 (* it251 (- 1))) 0) (= (+ it246 (- 4)) 0)))
(assert (= (+ it255 (- 2)) 0))
(assert (= (+ it256 (* it251 (- 1))) 0))
(assert (= (+ (* it252 (- 1)) it257) 0))
(assert (= (+ (* it8 (- 1)) it258) 0))
(assert (= (+ it259 (* it7 (- 1))) 0))
(assert (= (+ it260 (* it8 (- 1))) 0))
(assert (= (+ (* it7 (- 1)) it261) 0))
(check-sat)
