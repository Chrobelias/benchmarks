(set-info :status unsat) ; swine result
(set-logic QF_EIA)
(declare-fun it144 () Int)
(declare-fun i1 () Int)
(declare-fun it10 () Int)
(declare-fun it2141 () Int)
(declare-fun it2142 () Int)
(declare-fun it2143 () Int)
(declare-fun it2144 () Int)
(declare-fun it2145 () Int)
(declare-fun it6 () Int)
(declare-fun it2146 () Int)
(declare-fun it192 () Int)
(declare-fun it15 () Int)
(declare-fun it2150 () Int)
(declare-fun it2151 () Int)
(declare-fun it2152 () Int)
(declare-fun it2153 () Int)
(declare-fun it2154 () Int)
(declare-fun it11 () Int)
(declare-fun it2155 () Int)
(declare-fun it20 () Int)
(declare-fun it2168 () Int)
(declare-fun it2169 () Int)
(declare-fun it2170 () Int)
(declare-fun it2171 () Int)
(declare-fun it2172 () Int)
(declare-fun it17 () Int)
(declare-fun it2173 () Int)
(declare-fun it16 () Int)
(declare-fun it25 () Int)
(declare-fun it223 () Int)
(declare-fun it2177 () Int)
(declare-fun it2178 () Int)
(declare-fun it2179 () Int)
(declare-fun it2180 () Int)
(declare-fun it22 () Int)
(declare-fun it2181 () Int)
(declare-fun it2182 () Int)
(declare-fun it21 () Int)
(declare-fun it30 () Int)
(declare-fun it2196 () Int)
(declare-fun it2197 () Int)
(declare-fun it2198 () Int)
(declare-fun it2199 () Int)
(declare-fun it2200 () Int)
(declare-fun it27 () Int)
(declare-fun it2201 () Int)
(declare-fun it26 () Int)
(declare-fun it40 () Int)
(declare-fun it2214 () Int)
(declare-fun it2215 () Int)
(declare-fun it2216 () Int)
(declare-fun it2217 () Int)
(declare-fun it37 () Int)
(declare-fun it2218 () Int)
(declare-fun it2219 () Int)
(declare-fun it36 () Int)
(declare-fun it45 () Int)
(declare-fun it479 () Int)
(declare-fun it2232 () Int)
(declare-fun it2233 () Int)
(declare-fun it2234 () Int)
(declare-fun it2235 () Int)
(declare-fun it42 () Int)
(declare-fun it2236 () Int)
(declare-fun it41 () Int)
(declare-fun it2237 () Int)
(declare-fun it2239 () Int)
(declare-fun it2242 () Int)
(declare-fun it2243 () Int)
(declare-fun it2244 () Int)
(declare-fun it2245 () Int)
(declare-fun it2240 () Int)
(declare-fun it2246 () Int)
(declare-fun it2241 () Int)
(declare-fun it2247 () Int)
(declare-fun it2248 () Int)
(assert (and (> it10 0) (= (+ i1 (- 13)) 0) (> (+ it144 1) 0)))
(assert (= (+ it2141 (- 2)) 0))
(assert (= (+ (* it10 (- 1)) it2142) 0))
(assert (= (+ it2143 (- 2)) 0))
(assert (= it2144 0))
(assert (= (+ it2145 (* it144 (- 1))) 0))
(assert (= (+ it2146 (* it6 (- 1))) 0))
(assert (and (> it15 0)
     (>= (+ it2142 (* it15 (- 1))) 0)
     (>= (+ it2141 (- 2)) 0)
     (>= (+ (* it2141 (- 1)) 2) 0)
     (> (+ it2145 (* it2144 (- 1))) 0)
     (> (+ (* it192 (- 1)) it2145 (* it2144 (- 1)) 1) 0)
     (>= (+ it192 (- 1)) 0)
     (> (+ it2145 1) 0)
     (> it2142 0)
     (> (+ it2143 (- 1)) 0)))
(assert (= (+ (* it2141 (- 1)) it2150) 0))
(assert (= (+ it2151 (* it15 (- 1))) 0))
(assert (= (+ (+ 0 it2152) (* (** 2 it192) (+ 0 it2143) (+ 0 (- 1))))
   (+ 0 0)))
(assert (= (+ (* it192 (- 1)) it2153 (* it2144 (- 1))) 0))
(assert (= (+ it2154 (* it2145 (- 1))) 0))
(assert (= (+ it2155 (* it11 (- 1))) 0))
(assert (and (> it20 0)
     (>= (+ it2151 (* it20 (- 1))) 0)
     (> (+ it2152 1) 0)
     (= (+ it2150 (- 2)) 0)
     (> it2151 0)
     (<= (+ it2154 (* it2153 (- 1))) 0)))
(assert (= (+ it2168 (- 3)) 0))
(assert (= (+ (* it20 (- 1)) it2169) 0))
(assert (= it2170 0))
(assert (= (+ it2171 (* it2152 (- 1))) 0))
(assert (= (+ (* it17 (- 1)) it2172) 0))
(assert (= (+ (* it16 (- 1)) it2173) 0))
(assert (let ((a!1 (> (+ (+ (* it2170 (- 1)) it2171) (* it223 (- 1)) 1) 0)))
  (and (> it25 0)
       a!1
       (> it2169 0)
       (>= (+ it223 (- 1)) 0)
       (>= (+ it2169 (* it25 (- 1))) 0)
       (> (+ (* it2170 (- 1)) it2171) 0)
       (> (+ it2171 1) 0)
       (>= (+ it2168 (- 3)) 0)
       (> (+ it2170 1) 0)
       (>= (+ (* it2168 (- 1)) 3) 0))))
(assert (= (+ it2177 (* it2168 (- 1))) 0))
(assert (= (+ it2178 (* it25 (- 1))) 0))
(assert (= (+ (* it2170 (- 1)) it2179 (* it223 (- 1))) 0))
(assert (= (+ it2180 (* it2171 (- 1))) 0))
(assert (= (+ it2181 (* it22 (- 1))) 0))
(assert (= (+ (* it21 (- 1)) it2182) 0))
(assert (and (> it30 0)
     (= (+ it2177 (- 3)) 0)
     (> it2178 0)
     (<= (+ it2180 (* it2179 (- 1))) 0)
     (<= (+ it30 (* it2178 (- 1))) 0)
     (> it2180 0)))
(assert (= (+ it2196 (- 4)) 0))
(assert (= (+ (* it30 (- 1)) it2197) 0))
(assert (= it2198 0))
(assert (= (+ (* it2180 (- 1)) it2199) 0))
(assert (= (+ (* it27 (- 1)) it2200) 0))
(assert (= (+ (* it26 (- 1)) it2201) 0))
(assert (and (> it40 0)
     (> it2197 0)
     (<= (+ it40 (* it2197 (- 1))) 0)
     (= (+ it2196 (- 4)) 0)
     (> (+ it2199 (- 1)) 0)
     (> (+ (* it2198 (- 1)) it2199) 0)))
(assert (= (+ it2214 (- 5)) 0))
(assert (= (+ it2215 (* it40 (- 1))) 0))
(assert (= it2216 0))
(assert (= (+ (* it2199 (- 1)) it2217) 0))
(assert (= (+ it2218 (* it37 (- 1))) 0))
(assert (= (+ (* it36 (- 1)) it2219) 0))
(assert (and (> it45 0)
     (>= (+ it479 (- 1)) 0)
     (>= (+ (* it2214 (- 1)) 5) 0)
     (>= (+ it2214 (- 5)) 0)
     (> (+ (* it2216 (- 1)) it2217) 0)
     (> (+ it2217 1) 0)
     (> it2215 0)
     (> (+ (* it479 (- 1)) (* it2216 (- 1)) it2217 1) 0)
     (>= (+ (* it45 (- 1)) it2215) 0)))
(assert (= (+ (* it2214 (- 1)) it2232) 0))
(assert (= (+ (* it45 (- 1)) it2233) 0))
(assert (= (+ (* it479 (- 1)) (* it2216 (- 1)) it2234) 0))
(assert (= (+ (* it2217 (- 1)) it2235) 0))
(assert (= (+ it2236 (* it42 (- 1))) 0))
(assert (= (+ it2237 (* it41 (- 1))) 0))
(assert (and (> (+ it2235 1) 0)
     (> it2233 0)
     (> it2239 0)
     (<= (+ it2239 (* it2233 (- 1))) 0)
     (= (+ it2232 (- 5)) 0)
     (> (+ (* it2234 (- 1)) it2235) 0)))
(assert (= (+ it2242 (- 5)) 0))
(assert (= (+ (* it2239 (- 1)) it2243) 0))
(assert (= (+ (* it2234 (- 1)) it2244 (- 1)) 0))
(assert (= (+ (* it2235 (- 1)) it2245) 0))
(assert (= (+ it2246 (* it2240 (- 1))) 0))
(assert (= (+ it2247 (* it2241 (- 1))) 0))
(assert (and (>= (+ it2242 (- 5)) 0)
     (>= (+ (* it2242 (- 1)) 5) 0)
     (>= (+ it2243 (* it2248 (- 1))) 0)
     (> it2243 0)
     (> (+ it2245 1) 0)
     (> it2248 0)
     (> (+ (* it2244 (- 1)) it2245) 0)
     (= (- 1) 0)
     (= 0 0)))
(check-sat)
