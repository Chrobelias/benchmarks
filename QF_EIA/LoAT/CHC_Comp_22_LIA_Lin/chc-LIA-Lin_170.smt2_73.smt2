(set-info :status sat) ; swine result
(set-logic ALL)
(declare-fun i1 () Int)
(declare-fun it384 () Int)
(declare-fun it385 () Int)
(declare-fun it15 () Int)
(declare-fun it386 () Int)
(declare-fun it14 () Int)
(declare-fun it13 () Int)
(declare-fun it387 () Int)
(declare-fun bt433 () Bool)
(declare-fun bt434 () Bool)
(declare-fun bt435 () Bool)
(declare-fun bt436 () Bool)
(declare-fun it17 () Int)
(declare-fun it16 () Int)
(declare-fun it18 () Int)
(declare-fun bt25 () Bool)
(declare-fun bt28 () Bool)
(declare-fun bt26 () Bool)
(declare-fun bt27 () Bool)
(declare-fun it391 () Int)
(declare-fun it392 () Int)
(declare-fun it393 () Int)
(declare-fun it394 () Int)
(declare-fun bt437 () Bool)
(declare-fun bt438 () Bool)
(declare-fun bt439 () Bool)
(declare-fun bt440 () Bool)
(declare-fun it356 () Int)
(declare-fun it468 () Int)
(declare-fun it469 () Int)
(declare-fun it470 () Int)
(declare-fun it471 () Int)
(declare-fun bt509 () Bool)
(declare-fun bt510 () Bool)
(declare-fun bt511 () Bool)
(declare-fun bt512 () Bool)
(declare-fun it397 () Int)
(declare-fun bt442 () Bool)
(declare-fun bt441 () Bool)
(declare-fun it396 () Int)
(declare-fun bt443 () Bool)
(declare-fun bt444 () Bool)
(declare-fun it395 () Int)
(declare-fun it474 () Int)
(declare-fun it475 () Int)
(declare-fun it476 () Int)
(declare-fun it477 () Int)
(declare-fun bt513 () Bool)
(declare-fun bt514 () Bool)
(declare-fun bt515 () Bool)
(declare-fun bt516 () Bool)
(declare-fun it478 () Int)
(declare-fun it479 () Int)
(declare-fun bt519 () Bool)
(declare-fun bt517 () Bool)
(declare-fun bt518 () Bool)
(declare-fun it480 () Int)
(declare-fun bt520 () Bool)
(declare-fun it482 () Int)
(declare-fun it483 () Int)
(declare-fun it484 () Int)
(declare-fun it485 () Int)
(declare-fun bt521 () Bool)
(declare-fun bt522 () Bool)
(declare-fun bt523 () Bool)
(declare-fun bt524 () Bool)
(declare-fun it486 () Int)
(declare-fun bt527 () Bool)
(declare-fun bt528 () Bool)
(declare-fun bt526 () Bool)
(declare-fun bt525 () Bool)
(declare-fun it488 () Int)
(declare-fun it487 () Int)
(declare-fun it495 () Int)
(declare-fun it496 () Int)
(declare-fun it497 () Int)
(declare-fun it498 () Int)
(declare-fun bt537 () Bool)
(declare-fun bt538 () Bool)
(declare-fun bt539 () Bool)
(declare-fun bt540 () Bool)
(declare-fun bt541 () Bool)
(declare-fun bt542 () Bool)
(declare-fun it499 () Int)
(declare-fun it501 () Int)
(declare-fun bt544 () Bool)
(declare-fun bt543 () Bool)
(declare-fun it500 () Int)
(declare-fun it517 () Int)
(declare-fun it518 () Int)
(declare-fun it519 () Int)
(declare-fun it520 () Int)
(declare-fun bt565 () Bool)
(declare-fun bt566 () Bool)
(declare-fun bt567 () Bool)
(declare-fun bt568 () Bool)
(declare-fun it521 () Int)
(declare-fun bt571 () Bool)
(declare-fun bt569 () Bool)
(declare-fun bt572 () Bool)
(declare-fun it523 () Int)
(declare-fun it522 () Int)
(declare-fun bt570 () Bool)
(assert (= (+ i1 (- 1)) 0))
(assert (= (+ it384 (- 2)) 0))
(assert (= (+ (* it15 (- 1)) it385) 0))
(assert (= (+ (* it14 (- 1)) it386) 0))
(assert (= (+ it387 (* it13 (- 1))) 0))
(assert (or (not bt433) (and false bt433)))
(assert (or bt434 (and false (not bt434))))
(assert (or (not bt435) (and false bt435)))
(assert (or (not bt436) (and false bt436)))
(assert (and bt27
     (not bt26)
     (not bt28)
     (not bt25)
     (= (+ it18 (- 1)) 0)
     (= (+ it384 (- 2)) 0)
     bt434
     (not bt436)
     (not bt435)
     (not bt433)
     (= (+ it387 (* it16 (- 1))) 0)
     (= (+ (* it17 (- 1)) it386) 0)))
(assert (= (+ it391 (- 2)) 0))
(assert (= (+ (* it18 (- 1)) it392) 0))
(assert (= (+ (* it17 (- 1)) it393) 0))
(assert (= (+ it394 (* it16 (- 1))) 0))
(assert (or (and bt27 bt437) (and (not bt27) (not bt437))))
(assert (or (and (not bt28) (not bt438)) (and bt28 bt438)))
(assert (or (and bt26 bt439) (and (not bt26) (not bt439))))
(assert (or (and (not bt25) (not bt440)) (and bt25 bt440)))
(assert (let ((a!1 (+ (+ 0 it394)
              (* (exp 3 (+ it356 (- 1))) (+ 0 it392) (+ 0 (- 1))))))
  (and (>= (* it393 (- 1)) 0)
       (not bt438)
       bt437
       (>= (+ (* it391 (- 1)) 2) 0)
       (> a!1 (+ 0 0))
       (>= (+ it391 (- 2)) 0)
       (not bt440)
       (>= (+ it356 (- 1)) 0)
       (not bt439)
       (> (+ it394 (* it392 (- 1))) 0))))
(assert (= (+ it468 (* it391 (- 1))) 0))
(assert (= (+ (+ 0 it469) (* (+ 0 it392) (exp 3 it356) (+ 0 (- 1))))
   (+ 0 0)))
(assert (= (+ it470 (* it393 (- 1))) 0))
(assert (= (+ it471 (* it394 (- 1))) 0))
(assert (or bt509 (and false (not bt509))))
(assert (or (not bt510) (and false bt510)))
(assert (or (not bt511) (and false bt511)))
(assert (or (and false bt512) (not bt512)))
(assert (and (> (+ it471 (* it469 (- 1))) 0)
     (= (+ it469 (* it395 (- 1))) 0)
     (= (+ it468 (- 2)) 0)
     (not bt512)
     (not bt510)
     (not bt511)
     bt509
     (not bt444)
     (not bt443)
     (= (+ it470 (* it396 (- 1))) 0)
     bt441
     bt442
     (= (+ it471 (* it397 (- 1))) 0)))
(assert (= (+ it474 (- 2)) 0))
(assert (= (+ it475 (* it395 (- 1))) 0))
(assert (= (+ it476 (* it396 (- 1))) 0))
(assert (= (+ it477 (* it397 (- 1))) 0))
(assert (or (and (not bt442) (not bt513)) (and bt513 bt442)))
(assert (or (and bt514 bt441) (and (not bt514) (not bt441))))
(assert (or (and (not bt515) (not bt443)) (and bt515 bt443)))
(assert (or (and bt516 bt444) (and (not bt516) (not bt444))))
(assert (and (not bt520)
     (not bt515)
     (= (+ (* it480 (- 1)) it477) 0)
     bt518
     (not bt516)
     (<= it476 0)
     (not bt517)
     bt514
     (= (+ it474 (- 2)) 0)
     bt513
     bt519
     (= (+ (* it479 (- 1)) it476) 0)
     (= (+ (* it478 (- 1)) it475) 0)))
(assert (= (+ it482 (- 2)) 0))
(assert (= (+ (* it478 (- 1)) it483) 0))
(assert (= (+ it484 (* it479 (- 1))) 0))
(assert (= (+ (* it480 (- 1)) it485) 0))
(assert (or (and bt521 bt518) (and (not bt521) (not bt518))))
(assert (or (and bt522 bt517) (and (not bt522) (not bt517))))
(assert (or (and (not bt523) (not bt519)) (and bt523 bt519)))
(assert (or (and bt524 bt520) (and (not bt520) (not bt524))))
(assert (and (= (+ (* it487 (- 1)) it484) 0)
     bt521
     (= (+ (* it488 (- 1)) it485) 0)
     bt525
     bt526
     (not bt522)
     (not bt528)
     bt523
     bt527
     (= (+ it482 (- 2)) 0)
     (= (+ (* it486 (- 1)) (* it483 3)) 0)))
(assert (= (+ it495 (- 2)) 0))
(assert (= (+ it496 (* it486 (- 1))) 0))
(assert (= (+ (* it487 (- 1)) it497) 0))
(assert (= (+ (* it488 (- 1)) it498) 0))
(assert (or (and (not bt537) (not bt526)) (and bt526 bt537)))
(assert (or (and bt525 bt538) (and (not bt538) (not bt525))))
(assert (or (and bt539 bt527) (and (not bt527) (not bt539))))
(assert (or (and bt540 bt528) (and (not bt540) (not bt528))))
(assert (and (= (+ (* it500 (- 1)) it497) 0)
     bt539
     bt543
     (not bt544)
     (= (+ (* it501 (- 1)) it498) 0)
     (= (+ (* it499 (- 1)) it496) 0)
     (= (+ it495 (- 2)) 0)
     (not bt542)
     bt538
     bt541
     bt537))
(assert (= (+ it517 (- 2)) 0))
(assert (= (+ (* it499 (- 1)) it518) 0))
(assert (= (+ (* it500 (- 1)) it519) 0))
(assert (= (+ it520 (* it501 (- 1))) 0))
(assert (or (and bt565 bt542) (and (not bt565) (not bt542))))
(assert (or (and (not bt541) (not bt566)) (and bt566 bt541)))
(assert (or (and bt543 bt567) (and (not bt543) (not bt567))))
(assert (or (and (not bt568) (not bt544)) (and bt544 bt568)))
(assert (let ((a!1 (and (not bt570)
                (not bt569)
                (= (+ (* it522 (- 1)) it519) 0)
                (= (+ (* it523 (- 1)) it520) 0)))
      (a!3 (and (= (+ (* it522 (- 1)) it519) 0) (= (+ (* it523 (- 1)) it520) 0)))
      (a!5 (and (not bt569)
                (= (+ (* it522 (- 1)) it519) 0)
                (= (+ (* it523 (- 1)) it520) 0)))
      (a!7 (and (not bt570)
                (= (+ (* it522 (- 1)) it519) 0)
                (= (+ (* it523 (- 1)) it520) 0))))
(let ((a!2 (and a!1 (not bt572) bt571 (= (+ (* it521 (- 1)) it518) 0)))
      (a!4 (and (and a!3 (not bt571))
                bt569
                bt572
                bt570
                (= (+ (* it521 (- 1)) it518) 0)))
      (a!6 (and (and a!5 (not bt571))
                bt572
                bt570
                (= (+ (* it521 (- 1)) it518) 0)))
      (a!8 (and a!7 (not bt571) bt569 bt572 (= (+ (* it521 (- 1)) it518) 0)))
      (a!9 (and (and a!7 (not bt572) bt569 bt571)
                (= (+ (* it521 (- 1)) it518) 0)))
      (a!10 (and (and (and a!5 (not bt571)) (not bt572))
                 (= (+ it521 (- 1)) 0)
                 bt570))
      (a!11 (and (and a!1 (not bt571))
                 (not bt572)
                 (= (+ (* it521 (- 1)) it518) 0)))
      (a!12 (and a!5 (not bt572) bt571 bt570 (= (+ (* it521 (- 1)) it518) 0)))
      (a!13 (and (and a!3 (not bt571))
                 (not bt572)
                 bt569
                 bt570
                 (= (+ (* it521 (- 1)) it518) 0)))
      (a!15 (and (and (and a!5 (not bt571)) (not bt572))
                 bt570
                 (= (+ (* it521 (- 1)) it518) 0)))
      (a!16 (and (and a!1 (not bt571)) bt572 (= (+ (* it521 (- 1)) it518) 0)))
      (a!18 (and a!3
                 (not bt572)
                 bt569
                 bt571
                 bt570
                 (= (+ (* it521 (- 1)) (* it518 3)) 0)))
      (a!19 (and (and a!7 (not bt572) bt569 bt571)
                 (= (+ (* it521 (- 1)) (* it518 2)) 0))))
(let ((a!14 (or bt566
                a!13
                (<= (+ it520 (* it518 (- 1))) 0)
                bt567
                bt568
                (not bt565)))
      (a!17 (or bt566
                a!16
                (> (+ it520 (* it518 (- 1))) 0)
                bt567
                bt568
                (not bt565))))
  (and (or a!2 (<= it519 0) bt567 (not bt566) bt568 (not bt565))
       (or a!4 bt567 (not bt566) (not bt568) (not bt565))
       (or (or bt565 bt567) a!6 (not bt566) (not bt568))
       (or (or (or bt566 bt565) bt567) a!8 (not bt568) (<= it518 0))
       (or a!9 (not bt567) (not bt566) bt568 (not bt565))
       (or (or bt565 bt567) (not bt566) bt568 a!10)
       (= (+ it517 (- 2)) 0)
       (or (or bt566 bt565) a!4 bt567 (not bt568) (> it518 0))
       (or bt566 bt567 a!11 (not bt568) (not bt565))
       (or (> it519 0) bt567 a!12 (not bt566) bt568 (not bt565))
       a!14
       (or bt565 (not bt567) (not bt566) bt568 a!15)
       (or (or (or bt566 bt565) bt567) a!11 bt568)
       a!17
       (or bt566 a!18 (not bt567) bt568 (not bt565))
       (or (or bt566 bt565) (not bt567) bt568 a!19))))))
(check-sat)
